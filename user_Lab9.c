...
int ADCB2result=0;
int ADCBresults[16] = {0}; // storing oversampled vals
...
void ADChwifunc(void)
{
    ...
    ADCBresults[6] = AdcbResultRegs.ADCRESULT6;
    ADCBresults[7] = AdcbResultRegs.ADCRESULT7;
    ADCBresults[8] = AdcbResultRegs.ADCRESULT8;
    ADCBresults[9] = AdcbResultRegs.ADCRESULT9;
    ADCBresults[10] = AdcbResultRegs.ADCRESULT10;
    ADCBresults[11] = AdcbResultRegs.ADCRESULT11;
    ADCBresults[12] = AdcbResultRegs.ADCRESULT12;
    ADCBresults[13] = AdcbResultRegs.ADCRESULT13;
    ADCBresults[14] = AdcbResultRegs.ADCRESULT14;
    ADCBresults[15] = AdcbResultRegs.ADCRESULT15;

    char adc_i;
    long double avg_volts = 0;
    for(adc_i = 15; adc_i >= 0;adc_i--){
        avg_volts += (3.0*ADCBresults[adc_i]/4095.0);
    }
    avg_volts /= 16.0;
    ...
    if (!SIMU_beginnewdata) {// Only true if have not yet begun a message
        if (SIMU_checkfirstcommandbyte == 1) {
            ...
            if (0x7F == (unsigned char)data) {// Check for start char
                ...
                SIMU_Var2_toSIMU_32bit = 10000*(3.0*ADCBresults[0]/4095.0);
